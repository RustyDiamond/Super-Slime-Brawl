...
sprite1=[pygame.image.load('charactersprites/red'+str(x)+'.png').convert_alpha() for x in range(1,9)]
...
sprite2=[pygame.image.load('charactersprites/green'+str(x)+'.png').convert_alpha() for x in range(1,9)]
...
rocket_glow=[pygame.transform.smoothscale(pygame.image.load('weaponsprites/missile_glow'+str(x)+'.png').convert_alpha(),(int(22*1.7),int(52*1.7))) for x in range(1,11)]
...
shotgun_flashes=[[pygame.image.load('muzzleflashes/shotgun_flash_a'+str(x)+'.png').convert_alpha() for x in range(1,8)],\
					[pygame.image.load('muzzleflashes/shotgun_flash_b'+str(x)+'.png').convert_alpha() for x in range(1,8)]]
...
sniper_flashes=[[pygame.image.load('muzzleflashes/sniper_flash_a'+str(x)+'.png').convert_alpha() for x in range(1,8)]]
bazooka_smoke=[[pygame.image.load('muzzleflashes/bazooka_smoke'+str(x)+'.png').convert_alpha() for x in range(1,11)]]
...
snow_button_on=pygame.image.load('ui/snow_button_on.png').convert_alpha()
snow_button_off=pygame.image.load('ui/snow_button_off.png').convert_alpha()
...
green_winscreen=pygame.image.load('ui/green_winscreen.png').convert_alpha()
pink_winscreen=pygame.image.load('ui/pink_winscreen.png').convert_alpha()
drawscreen=pygame.image.load('ui/drawscreen.png').convert_alpha()
...
inverted_pistol_sound=pygame.mixer.Sound('sounds/inverted_pistol.wav')
inverted_pistol_sound.set_volume(0.35)
...
death_sounds=[pygame.mixer.Sound('sounds/death1.wav'),pygame.mixer.Sound('sounds/death2.wav')]
...
class Player:
	def __init__(self,xpos,ypos,idlesprites,runsprites,jumpsprite,squishsprite,scoresprite,hurtsprite,scoreboard,color,collision_map,controls='',weapon=False,weapon_sprite=False,face=False,\
		...
		self.knockback=0
		self.reloadtimes={"BAZOOKA":[1,0],"MINIGUN":[0.22,0],"PISTOL":[0.5,0],"SNIPER":[1.5,0],"SHOTGUN":[0.9,0]}
		self.recoil=[0,0]
		self.flame=False
		self.laser=False
		self.angle=0
	...
	def update(self,auto=False,autoright=False,autoleft=False,autojump=False,autofire=False):
			...
			if not pause:
				...
				if keys[self.controls[2]] and not self.stay or autojump:
					...
					if not self.jump and time.monotonic()-self.coyote<0.14:
						play_sound(random.choice(jumping_sounds),jumpchannel)
		...
		if self.rect[0]<-600 or self.rect[0]>2200 or self.rect[1]>screenheight+200:
			screenshake=("weapon",time.monotonic(),7)
			...
			if self.rect[0]<-600:
				if not auto:
					for k in range(30):
						...
						self.particles.append(Blood(self.rect,"RIGHT",self.color,random.randrange(5,12),rand=(0,12)))
				else:
					for k in range(30):
						self.particles.append(Blood(self.rect,"RIGHT",self.color,random.randrange(5,12),rand=(0,12),collision_map=self.collision_map))
			elif self.rect[0]>2200:
				if not auto:
					for k in range(30):
						# t,side,color,radius,decay=0.008,rand=(2,7))
						self.particles.append(Blood(self.rect,"LEFT",self.color,random.randrange(5,12),rand=(0,12)))
				else:
					for k in range(30):
						self.particles.append(Blood(self.rect,"LEFT",self.color,random.randrange(5,12),rand=(0,12),collision_map=self.collision_map))
			...
			if self.yc>0:
				...
				if self.state!="GROUNDED":
					if self.yc>10:						
						...
						play_sound(landing_sounds[0])
					else:
						# landing_sounds[1].play()
						play_sound(landing_sounds[1])
				self.state="GROUNDED"
		...
		if not auto:
			...
			if not self.weapon:
				for ob in weapons:
					wprct=weapons[ob][1].copy()
					cent=wprct.center
					wprct.size=(200,200)
					wprct.center=cent
					if self.rect.colliderect(wprct):
						name=ob
						if ob[-1]=='1':
							name=name[:len(name)-1]
						render_text(name,(weapons[ob][1].center[0]-scroll[0],weapons[ob][1][1]-10-scroll[1]),0.4)	
		...
		if not pause:
			if keys[self.controls[3]] or autofire:
				if self.weapon in ("BAZOOKA","BAZOOKA1"):
					
					if time.monotonic()-self.reloadtimes["BAZOOKA"][1]>self.reloadtimes["BAZOOKA"][0]:
						if not screenshake:
						...
						if self.rect[0]>self.opp.rect[0]:
							...
							self.particles.append(MuzzleFlash(bazooka_smoke,self.rect.center[0],self.rect.center[1],angle=[self.recoilangle-180]))
							self.recoil=vec(11,0).rotate(self.recoilangle*-1)
							offset=vec(-100,0).rotate(self.recoilangle*-1)

						else:
							# angle=math.atan2(-( self.opp.rect.center[1]-(self.rect.center[1]) ),( self.opp.rect.center[0]-(self.rect.center[0]) ))
							self.recoilangle=math.degrees(angle)-180
							self.particles.append(MuzzleFlash(bazooka_smoke,self.rect.center[0],self.rect.center[1],angle=[self.recoilangle]))
							self.recoil=vec(-11,0).rotate(self.recoilangle*-1)
							offset=vec(100,0).rotate(self.recoilangle*-1)

						self.bullets.append(RocketLauncher(pygame.Rect(self.rect.center[0]+offset[0],self.rect.center[1]+offset[1],rocket[0].get_width(),rocket[0].get_height())))

						self.reloadtimes["BAZOOKA"][1]=time.monotonic()
					...
					if time.monotonic()-self.reloadtimes["SHOTGUN"][1]>self.reloadtimes["SHOTGUN"][0]:
						...
						self.acc=0
						if self.face=="RIGHT":
							self.particles.append(MuzzleFlash(shotgun_flashes,self.rect[0]+45+10+25-100,self.rect.center[1]+15))
							for k in range(4):
							...
							self.particles.append(MuzzleFlash(shotgun_flashes,self.rect[0]-30+100,self.rect.center[1]+15,xflip=True))
							for k in range(4):
					...
					if time.monotonic()-self.reloadtimes["PISTOL"][1]>self.reloadtimes["PISTOL"][0]:
						...
						play_sound(random.choice(laser_sound))
						self.acc/=3
						if self.face=="RIGHT":
					...
					if time.monotonic()-self.reloadtimes["SNIPER"][1]>self.reloadtimes["SNIPER"][0]:
						...
						if self.rect[0]>self.opp.rect[0]:
							...
							self.recoil=vec(12,0).rotate(self.recoilangle*-1)
							offset=vec(-60,0).rotate(self.recoilangle*-1)
								# self.recoil[0]=8
						...
						self.bullets.append(Sniper(pygame.Rect(self.rect.center[0]+offset[0],self.rect.center[1]+offset[1],minigun_b.get_width(),minigun_b.get_height()),self.opp.rect,minigun_b))
	...
	##############################################################################################################################################################################
		...
		if pause:
			...
			if self.squishtime and time.monotonic()-self.squishtime<0.16 and time.monotonic()-self.squishtime>0.02:
				...
				self.sprite=self.idlesprites[int(self.idleframe)]
				self.offsetx=0
...
class RocketLauncher:
	def __init__(self,rect,sprite=rocket,glow=rocket_glow,mask=pygame.mask.from_surface(rocket[0])):
		...
		self.glow_rect=glow[0].get_rect(center=self.rect.center)
		self.vel = vec(0,0)
		self.acc = vec(0, 0)
	...
	def update(self,opp):
		...
		if not pause:
			...
			glow=self.glow[int(self.gframe)]
			self.rot_sprite=pygame.transform.rotate(sprite,angle)
			self.rot_glow=pygame.transform.rotate(glow,angle)
			...
			if self.rect.colliderect(opp.rect):
				...
				if self.vel[0]>0:
					...
					for x in range(4):
						opp.particles.append(Explosion((self.rect[0]+self.rect[2]+random.choice([x for x in range(-15,15,2)]),random.randrange(self.rect[1]+12,self.rect[1]+self.rect[3]-12)),True,size="BAZOOKA"))
				elif self.vel[0]<0:
					for x in range(30):
						opp.particles.append(Particle(self.rect,'LEFT',dx=[x for x in range(-1,8,2)],color=(194,167,55)))
					for x in range(4):
						opp.particles.append(Explosion((self.rect[0]+random.choice([x for x in range(-15,15,2)]),random.randrange(self.rect[1]+12,self.rect[1]+self.rect[3]-12)),True,size="BAZOOKA"))

				self.alive=False

		screen.blit(self.rot_glow,(int(self.glow_rect[0]-scroll[0]),int(self.glow_rect[1]-scroll[1])))
		screen.blit(self.rot_sprite,(int(self.rect[0]-scroll[0]),int(self.rect[1]-scroll[1])))
...
class InvertedPistol:
	...
	def update(self,opp):
		...
		if not pause:
			...
			if self.rect[0]>3000 or self.rect[0]<-1500 or self.rect[1]>2000 or self.rect[1]<-2000:
				self.alive=False
			...
			if self.right==-1 and self.rect.center[0]>self.player.rect.center[0]-450:
				...
				play_sound(inverted_pistol_sound)
				self.right=0
				# self.alive=False

			elif self.right==1 and self.rect.center[0]<self.player.rect.center[0]+450:
	...
	def update(self,opp):
		...
		if not pause:
			if self.distance>random.randint(75,250):
				...
				self.distance+=x
				self.rect[1]+=random.choice([x for x in range(-12,8)])
			else:
...
class Blood:
	def __init__(self,t,side,color,radius=False,decay=0.008,rand=(2,7),collision_map=False):
		...
		self.decay=decay
		self.vel=random.randint(rand[0],rand[1])*random.choice([-1,-0.8,-1.2])
		...
		if side=='RIGHT':
			...
			if radius:
				...
				self.center[0]+=random.randrange(-30,31,3)
			else:
			...
			if radius:
				self.dx=random.choice([[x for x in range(-3,1,1)],[x for x in range(0,4,1)],[0.2,-0.2]])
...
class WinScreen:
	def __init__(self,win):
		if win=='green':
			self.sprite=green_winscreen
		elif win=='pink':
			self.sprite=pink_winscreen
		elif win=='draw':
			self.sprite=drawscreen

		self.rect=self.sprite.get_rect(right=0,top=(65)*4)
		self.vec=vec(self.rect[0],self.rect[1])
	...
	def update(self):
		...
		# self.rect.topleft=(self.vec[0],self.vec[1])
		...
		if self.rect.center[0]>=screenwidth//2-80:
			if self.rect.center[0]>=screenwidth//2+80:
				self.rect[0]+=38
			else:
				self.rect[0]+=1
		...
		# 	self.rect[1]-=33/(384*1000000000000)
			# print(self.rect[1])
		# if self.rect[1]<-100:
			# self.rect[1]=-100
...
def play_sound(sound,channel=False):
	global soundcheck
	if soundcheck:
		if channel:
...
def MainMenu(transout=False):
	global particlecheck,trailcheck,snowcheck,pause,play1controls,play2controls,scroll
	...
	snow_button_rect=snow_button_on.get_rect(center=((screenwidth/2)-50,(screenheight/2)-30))
	particle_button_rect=particle_button_on.get_rect(top=snow_button_rect.bottom+20,left=snow_button_rect.left)
	...
	platform=[gooey_rect,warfare_rect]
	buttons=[]
	...
	while running:
				...
				if event.button==1:
					if click:
						...
						if selected=="PLAY":
							...
							snowcheck=not snowcheck
						elif selected=="PARTICLES":
							particlecheck=not particlecheck
						elif selected=="TRAILS":
							trailcheck=not trailcheck
		...
		if snowcheck:
			snowfall=random.choice([1,2,3,4,4,4,4,4])
			if snowfall==1:
		...
		if not settingscreen and not modescreen:
			gooey_glow.set_alpha(int(glow_alpha))
			gooey.set_alpha(title_alpha)
			warfare_glow.set_alpha(int(glow_alpha))
			...
			buttons=[play_button_rect,settings_button_rect,quit_button_rect]
			...
			buttons=[deathmatch_button_rect,showdown_button_rect,escape_button_rect]
			...
			buttons=[particle_button_rect,trails_button_rect,snow_button_rect,escape_button_rect]
			render_text("video settings",(screenwidth/2+8,80),2)
			if snow_button_rect.collidepoint(mousepos):
				selected="SNOW"
			elif particle_button_rect.collidepoint(mousepos):
				selected="PARTICLES"
			elif trails_button_rect.collidepoint(mousepos):
				selected="TRAILS"

			if snowcheck:
				screen.blit(snow_button_on,snow_button_rect)
			else:
				screen.blit(snow_button_off,snow_button_rect)
			...
			buttons=[video_button_rect,controls_button_rect,escape_button_rect]
...
def Game(gamemode):
	...
	global world,screenshake,weapons,avail_weapons,pause,particlecheck,trailcheck,snowcheck,play1controls,play2controls,win,wintime
	snow=[]
	snowy=-200
	avail_weapons=[["MINIGUN",minigun,False],['SNIPER',sniper,False],['BAZOOKA',bazooka,False],["PISTOL",deagle,False],\
		...
		for event in pygame.event.get():
			if event.type==pygame.QUIT:
				...
				# 	play2.alive=True
					...
					snow_button_rect=snow_button_on.get_rect(center=(screenwidth/2-50,screenheight/2+20))
					particle_button_rect=particle_button_on.get_rect(top=snow_button_rect.bottom+20,left=snow_button_rect.left)
					trails_button_rect=trails_button_on.get_rect(top=particle_button_rect.bottom+20,left=particle_button_rect.left)
					video_button_rect=video_button.get_rect(center=(screenwidth/2,screenheight/2))
					controls_button_rect=controls_button.get_rect(centerx=screenwidth/2,top=video_button_rect.bottom+20)
					...
					while pause:
						...
						if not settingscreen:
							buttons=[continue_button_rect,settings_button_rect,quit_button_rect,restart_button_rect]
			...
			if time.monotonic()-play2.deathtime>1.7:
				...
				if play1.score<5:
					...
					if not win:
						if not wintime:	
							wintime=time.monotonic()
						if time.monotonic()-wintime<5:
							if play2.score==5 and not play1.alive:
								win=WinScreen('draw')
								wintime=time.monotonic()
						else:
							win=WinScreen('pink')
			...
			if time.monotonic()-play1.deathtime>1.7:
				...
				if play2.score<5:
					...
					if not win:
						...
						if not wintime:	
							wintime=time.monotonic()
						if time.monotonic()-wintime<3:
							if play1.score==5 and not play2.alive:
								win=WinScreen('draw')
								wintime=time.monotonic()
						else:
							win=WinScreen('green')
